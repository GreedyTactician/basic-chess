  	...
  	if k == "board" then
		...
		if k == "play" then
			...
			return function(ply)
				...
				--apply the move to that state
				applyply(t.current_state.board, ply, listeners)
				--add the move the move history (or as I call it: ply)
				table.insert(t.current_state.ply_history, ply)
			end
		end
		if k == "undo" then
			return function()
				if #ret.state_history > 0 then
					t.current_state = table.remove(ret.state_history)
				end
			end
		end
