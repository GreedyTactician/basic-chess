function init(funcs, width, height)	
	...
	local function draw_board(board, xsize, ysize, quadimage, piece_to_quads, piecesize, prepiecetodo, postpiecetodo)
		...
		for x = 0, 7 do
		  for y = 0, 7 do
		  	DRAW_IMAGE(quadimage, x*xsizesquare+xcenteroffset, (ysize-realypiecesize)-(y*ysizesquare+ycenteroffset), xpiecescale, ypiecescale, piece_to_quads[board[{x, y}]])
	...
	local function to_game_coordinates(x, y, xsize, ysize)
		local xsizesquare = xsize/8
		local ysizesquare = ysize/8

		return math.floor(x/xsizesquare), 7-math.floor(y/ysizesquare)
	end

	local function to_screen_coordinates(x, y, xsize, ysize)
		local xsizesquare = xsize/8
		local ysizesquare = ysize/8


		return x*xsizesquare, ysize-(y+1)*ysizesquare	
	end
	...
	--  local on_screen_square_clicked = {math.floor(8*x/BOARDSIZE[1]), math.floor(8*y/BOARDSIZE[2])}
		...
		board_clicked(to_game_coordinates(x, y, BOARDSIZE[1], BOARDSIZE[2]))

	end
	
	return {mousepressed = mousepressed, draw = draw, updated = function() return UPDATED end}
end
